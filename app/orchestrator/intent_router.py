...
## Skill区分规则（重要！）
- **policy_qa** vs **project_query**: policy_qa用于政策/制度/规范/方法论的问答（如"什么是关键路径法"、"XX规定是什么"），project_query用于具体项目情况查询（如"项目人员配置"、"进度偏差"）
- **cost_accounting** vs **cost_monitor**: cost_accounting用于成本核算和报表生成（涉及"核算"、"结算"、"报表"），cost_monitor用于成本监控和预警（涉及"监控"、"超预算"）
- **project_overview** vs **project_query**: project_overview用于查看项目整体状态概览，project_query用于具体项目问题咨询
...
- candidate_skills: 候选Skill列表，格式为 {{"skill": "skill名称", "reason": "匹配原因"}}（仅当intent_type为ambiguous时填写）
...
{{
  ...
  "confidence": 0.5,
  "candidate_skills": [
    {{"skill": "project_overview", "reason": "用户可能想查看项目状态"}},
    {{"skill": "project_query", "reason": "用户可能在询问项目问题"}}
  ],
  "extracted_params": {{}},
  "missing_params": []
}}
...
class IntentRouter:
    ...
    async def recognize(
        ...
        try:
            ...
            # 构建IntentResult
            # 处理candidate_skills格式兼容
            candidate_skills = self._normalize_candidate_skills(
                parsed_result.get("candidate_skills", [])
            )
    ...
    def _build_skill_descriptions(
        ...
        manifests = self._skill_registry.list_all_skills()
        ...
        if available_skills:
            for skill_name in available_skills:
                if skill_name in descriptions:
                    ...
                    params_info = self._get_params_info(skill_name, manifests)
    ...
    def _get_params_info(self, skill_name: str, manifests: List[Any]) -> str:
        """
        获取Skill参数信息。

        Args:
            skill_name: Skill名称
            manifests: Manifest列表
        ...
        for param_name, param_def in properties.items():
            ...
            params_str.append(f"{param_name}({req}: {desc})")
    ...
    def _normalize_candidate_skills(
        ...
        """
        规范化candidate_skills格式.
        
        兼容LLM返回的两种格式：
        - 字典列表: [{"skill": "name", "reason": "..."}]
        - 字符串列表: ["skill_name", "skill_name2"]
        
        Args:
            candidates: 原始候选列表
            
        Returns:
            List[Dict[str, Any]]: 规范化后的字典列表
        """
        normalized = []
        for item in candidates:
            if isinstance(item, dict):
                ...
                normalized.append({
                    "skill": item,
                    "reason": f"可能匹配的Skill: {item}"
                })
            else:
                logger.warning(
                    "Unexpected candidate_skill format",
    ...
    async def quick_match(
        ...
        """
        # 关键词映射 - 注意：更精确的关键词应放在前面
        keyword_mappings = {
            # 成本相关Skill关键词（优先匹配更精确的关键词）
            "cost_estimation": [
    ...
    async def recognize_with_context(
        ...
        if user_context.current_project:
            ...
            try:
                ...
                logger.warning("Failed to get available skills for user", error=str(e))
                available_skills = list(skill_registry.get_skill_names())
        ...
        try:
            ...
            logger.warning("LLM intent recognition failed, using fallback", error=str(e))
            # LLM失败时返回默认回复
            return IntentResult(
