...
class LarkSheetSyncService:
    ...
    async def _read_sheet_data(
        ...
        try:
            ...
            # 调用飞书API读取表格数据
            # 使用LarkClient的request方法调用飞书表格API
            endpoint = f"/open-apis/sheets/v2/spreadsheets/{binding.lark_sheet_token}/values/{binding.lark_sheet_id}"
            response = await client.request(
                "GET",
                endpoint,
                params={"range": sheet_range}
            )
            ...
            for row_idx, row in enumerate(data_rows):
                ...
                # 添加飞书表格行号
                row_data["_lark_sheet_row"] = row_idx + 2  # 从第2行开始
    ...
    async def _update_project(
        ...
        project.sync_version = getattr(project, "sync_version", 0) + 1
        ...
        if module == "project":
            ...
            if project:
                return [{
                    ...
                    "pm_name": getattr(project, "pm_name", None),
                    "department_name": getattr(project, "department_name", None),
                }]
            return []
    ...
    async def _write_sheet_data(
        ...
        try:
            ...
            # 调用飞书API写入
            endpoint = f"/open-apis/sheets/v2/spreadsheets/{binding.lark_sheet_token}/values"
            payload = {
                "sheetId": binding.lark_sheet_id,
                "range": sheet_range,
                "values": values,
            }
            response = await client.request("PUT", endpoint, data=payload)

            if response.get("code") != 0:
                logger.warning(f"Write to sheet returned error: {response.get('msg')}")
