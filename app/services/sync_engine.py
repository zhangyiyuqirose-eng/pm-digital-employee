...
class SyncEngine:
    ...
    async def get_active_bindings(
        ...
        """
        query = select(LarkSheetBinding).where(
            and_(
                LarkSheetBinding.project_id == project_id,
                LarkSheetBinding.sync_enabled == True,
                LarkSheetBinding.status == "active",
            )
        )

        if module:
            query = query.where(LarkSheetBinding.module == module)
